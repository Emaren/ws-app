...
export default function FloatAd({
  ...
  React.useEffect(() => {
    ...
    const eventName = `ws-floatad-open:${eventKey}`;
    const handleOpen = () => openDeliveryDialog();

    window.addEventListener(eventName, handleOpen);
    return () => window.removeEventListener(eventName, handleOpen);
  }, [eventKey, openDeliveryDialog, triggerOnly]);
  ...
  const triggerButton = triggerOnly ? null : (
    <button
      id={buttonId}
      type="button"
  ...
  const dialogMarkup = isDialogOpen ? (
    <div
