...
test("mobile header keeps only the logo rail, theme circles, and menu visible when closed", async ({
  ...
  expect(geometry.themeTop ?? 0).toBeLessThanOrEqual((geometry.menuRect?.bottom ?? 0) + 18);
  expect(geometry.themeTop ?? 0).toBeLessThan((geometry.logoRect?.bottom ?? Infinity) + 1);
  expect(geometry.themeRight ?? 0).toBeLessThanOrEqual((geometry.menuRect?.right ?? 0) + 1);
